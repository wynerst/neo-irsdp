...
<div class="historybox">
	...
	<?php 
	//cek jika project_flow DONE atau belum
	...
	<?php 
		...
		//cek jika kontrak sudah beres untuk semua pembayaran atau belum
		//tahapan ini dilewat, karena tidak sesuai dengan kebutuhan
		...
		<?php
			...
			<?php 
				if($next_step_id==0) 
					echo "Tender Processes"; /* FIXME */
				else 
					echo $next_step->row('kode_status').' - '.$next_step->row('detil_status');
			...
			<table id="tabdata" width="100%" class="nicetable">
				...
				<tbody>
					<tr>
						...
						<td><input id="1_0" name="1_0" type="text" style="width:200px" /></td>
					</tr>
				</tbody>
			</table>
			<?php endif;?>
...
?>
	<div class="historybox">
		<h3>Administrative Fulfillment</h3>
		<?php 
		// Count problem
...
<div class="historybox">
	...
	<?php foreach($cerita->result() as $cer): ?>	
		<tr class="<?php 
			...
			<td align="center"><?php 
					...
					{
						echo "<br /><a href='".site_url('petugas/set_problem_status/'.$detil->row('idproject_profile').'/'.$detil->row('idproj_flow').'/'.$cer->idstatusproject)."'><img src='".$this->config->item('icon_path').'wand.png'."' style=\"vertical-align:text-bottom;\"  />&nbsp;Close Problem</a>";
					}
					?>
			</td>
		</tr>
	...
	<div class="commentbox">
		...
		<div class="commentbox">
			...
			<input type="submit" name="submit_dokumen" value="Upload Document" />		
			<?php echo form_close();?>
		</div>
		<?php endif;?>
	</div>
	...
	<div class="commentbox">
		...
		<input type="submit" name="submit_komentar" value="Submit Description" />		
		<?php echo form_close();?>
	</div>
	<?php endif;?>
</div>