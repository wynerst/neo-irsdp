...
class simbio_datagrid extends simbio_table
{
    ...
    public function createDataGrid($obj_db, $str_db_table = '', $int_num2show = 30, $bool_editable = false)
    {
        ...
        if (!isset($this->chbox_property)) $this->chbox_property = array('itemID', 'DELETE');
        if (!isset($this->edit_property)) $this->edit_property = array('itemID', 'EDIT');
        ...
        if (!$this->chbox_confirm_msg) {
            $this->chbox_confirm_msg = 'Are You Sure Want to DELETE Selected Data?';
        }
        ...
        // check group by
        if ($this->sql_group) {
            $this->sql_group_by = ' GROUP BY '.$this->sql_group;
        }
     ...
     * @param   string  $str_order_column
     */
    public function setSQLgroup($str_group_column)
    {
        if (!$str_group_column) {
            // do nothing
        } else {
            // remove WHERE word if exist
            $this->sql_group = $str_group_column;
        }
    }
